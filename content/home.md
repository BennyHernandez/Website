---
portfolio:
  - title: Guys and Dolls
    type: Lighting Design
    role: Associate Lighting Designer
    date: 2024
    location: Sierra College
    crew:
      - name: Scott Adams
        role: Director
      - name: Michelle Zimmer
        role: Lighting Designer
    description: Created lighting looks, pallettes, presets, magic sheets, and
      Lightwright paperwork. Installed practicals on set pieces using external
      dimmer pack and wireless DMX. Managed spot operators, created spot sheet,
      and hung & focused lights.
    photo:
      - image: /GuysDolls2.jpg
        caption: ""
      - image: /GuysDolls.jpg
        caption: ""
      - image: /HotBox.jpg
        caption: Hot box lighting w/ 4 sets of individually controlled strands
      - image: /HotBox2.jpg
        caption: Back of Hot Box wagon. Wiring for practical lighting.
      - image: /HotBox3.jpg
        caption: Dimmer pack and wireless DMX used to control practical lighting on Hot
          Box wagon
      - image: /Pillar.jpg
        caption: Pillar up lighting using blackwrap to keep light off the ceiling
  - title: Romeo & Juliet
    type: Lighting Design
    role: Assistant Lighting Designer
    date: 2025
    location: The Sacramento Ballet
    crew:
      - name: Benjamin Gantose
        role: Lighting Designer
    description: Created spot sheets & other paper work, instructed spots, aided
      local crew in focusing, facilitated communications with other departments,
      and took notes for designer, stage manager, and choreographer.
    photo:
      - image: /R&J2.jpg
        caption: Programing at the SAFE Credit Union Performing Arts Center
      - image: /R&J.jpg
        caption: ""
  - title: Romeo & Juliet
    type: Lighting Design
    role: Assistant Lighting Designer
    date: 2025
    location: Sierra College
    crew:
      - name: Scott Adams
        role: Director
      - name: Michelle Zimmer
        role: Lighting Designer
    description: Instructed Stage Lighting class on hang & focus for this
      production, completed patching & addressing work, created palettes,
      presets & magic sheets, wired and installed torch practicals.
    photo:
      - image: /R&JSierra2.jpg
        caption: ""
      - image: /R&JSierra.jpg
        caption: ""
  - title: The Pops Chorale
    type: Lighting Design
    role: Lighting Designer
    date: 2024
    location: Sierra College
    crew: []
    description: Created lighting looks based on music, used busking techniques,
      created EOS effects, and managed spot operators. Credits include Holiday
      Show '24, Big Band '24, American Pops '24, and Big Band '23.
    photo:
      - image: /Pops.png
        caption: ""
      - image: /ConcertEOS.png
        caption: ""
  - title: Miss Sacramento County
    type: Lighting Design
    role: Lighting Designer
    date: 2024
    location: Sacramento County
    crew: []
    description: Created lighting looks & effects synced with music, utilized venue
      equipment to program and run show (2023, 2024).
    photo:
      - image: /MissSac.jpg
        caption: ""
  - title: Carrie The Musical
    type: Lighting Design
    role: Assistant Lighting Designer
    date: 2025
    location: California Repertory Company
    crew:
      - name: Daniel Nakawatasae
        role: Director
      - name: Martha Carter
        role: Lighting Designer
      - name: Ryan Meissner
        role: Programmer
    description: "Worked with Martha to assemble paper work such as spot sheets, cue
      sheets, preset list and more to make designing and programming the show
      possible. Also helped design effects, specific scenes, and work to solve
      technical problems. "
    photo:
      - image: img/carrie.jpg
      - image: img/screenshot-2026-03-02-at-5.38.29 pm.png
        caption: Spot sheets
---
